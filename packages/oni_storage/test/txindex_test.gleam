...
pub fn default_config_disabled_test() {
  let config = txindex.TxIndexConfig(
    data_dir: "/tmp/test",
    enabled: False,
  )
...
pub fn tx_location_creation_test() {
  ...
  let location = txindex.TxLocation(
  ...
  should.equal(location.block_height, 100000)
}
...
pub fn index_block_transactions_empty_test() {
  let entries = txindex.index_block_transactions(
    create_test_block_hash(),
    100,
    [],
  )
