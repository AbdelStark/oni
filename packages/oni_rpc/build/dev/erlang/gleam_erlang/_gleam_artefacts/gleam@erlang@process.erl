...
-opaque subject(FJQ) :: {subject, pid_(), gleam@erlang:reference_()} |
    {gleam_phantom, FJQ}.

-type do_not_leak() :: any().

-type selector(FJR) :: any() | {gleam_phantom, FJR}.
...
-type call_error(FJS) :: {callee_down, gleam@dynamic:dynamic_()} |
    call_timeout |
    {gleam_phantom, FJS}.
...
-file("src/gleam/erlang/process.gleam", 14).
?DOC(" Get the `Pid` for the current process.\n").
-spec self() -> pid_().
self() ->
    erlang:self().

-file("src/gleam/erlang/process.gleam", 28).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 110).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 164).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 182).
?DOC(
...
-spec select(selector(FKJ), integer()) -> {ok, FKJ} | {error, nil}.
select(From, Within) ->
    gleam_erlang_ffi:select(From, Within).

-file("src/gleam/erlang/process.gleam", 191).
?DOC(
...
-spec select_forever(selector(FKN)) -> FKN.
select_forever(From) ->
    gleam_erlang_ffi:select(From).

-file("src/gleam/erlang/process.gleam", 200).
?DOC(
...
-spec map_selector(selector(FKP), fun((FKP) -> FKR)) -> selector(FKR).
map_selector(A, B) ->
    gleam_erlang_ffi:map_selector(A, B).

-file("src/gleam/erlang/process.gleam", 209).
?DOC(
...
-spec merge_selector(selector(FKT), selector(FKT)) -> selector(FKT).
merge_selector(A, B) ->
    gleam_erlang_ffi:merge_selector(A, B).

-file("src/gleam/erlang/process.gleam", 253).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 225).
?DOC(
...
-spec selecting_trapped_exits(selector(FKX), fun((exit_message()) -> FKX)) -> selector(FKX).
...
-file("src/gleam/erlang/process.gleam", 263).
?DOC(
...
-spec selecting(selector(FLA), subject(FLC), fun((FLC) -> FLA)) -> selector(FLA).
...
-file("src/gleam/erlang/process.gleam", 128).
?DOC(
...
-spec 'receive'(subject(FKD), integer()) -> {ok, FKD} | {error, nil}.
...
-file("src/gleam/erlang/process.gleam", 279).
?DOC(
...
-spec selecting_record3(
    selector(FLJ),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FLJ)
) -> selector(FLJ).
...
-file("src/gleam/erlang/process.gleam", 313).
?DOC(
...
-spec selecting_record4(
    selector(FLN),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FLN)
) -> selector(FLN).
...
-file("src/gleam/erlang/process.gleam", 331).
?DOC(
...
-spec selecting_record5(
    selector(FLR),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FLR)
) -> selector(FLR).
...
-file("src/gleam/erlang/process.gleam", 349).
?DOC(
...
-spec selecting_record6(
    selector(FLV),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FLV)
) -> selector(FLV).
...
-file("src/gleam/erlang/process.gleam", 367).
?DOC(
...
-spec selecting_record7(
    selector(FLZ),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FLZ)
) -> selector(FLZ).
...
-spec selecting_record8(
    selector(FMD),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FMD)
) -> selector(FMD).
...
-file("src/gleam/erlang/process.gleam", 438).
?DOC(
...
-spec selecting_anything(selector(FMH), fun((gleam@dynamic:dynamic_()) -> FMH)) -> selector(FMH).
selecting_anything(Selector, Handler) ->
    gleam_erlang_ffi:insert_selector_handler(Selector, anything, Handler).

-file("src/gleam/erlang/process.gleam", 456).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 463).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 472).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 503).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 512).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 548).
?DOC(
...
-spec try_call(subject(FMS), fun((subject(FMU)) -> FMS), integer()) -> {ok, FMU} |
    {error, call_error(FMU)}.
...
-file("src/gleam/erlang/process.gleam", 587).
?DOC(
    " Send a message to a process and wait for a reply.\n"
    "\n"
    " If the receiving process exits or does not reply within the allowed amount\n"
...
-spec call(subject(FMZ), fun((subject(FNB)) -> FMZ), integer()) -> FNB.
call(Subject, Make_request, Timeout) ->
    Resp@1 = case try_call(Subject, Make_request, Timeout) of
        {ok, Resp} -> Resp;
        _assert_fail ->
...
-file("src/gleam/erlang/process.gleam", 606).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 613).
...
-file("src/gleam/erlang/process.gleam", 625).
?DOC(" Send a message over a channel after a specified number of milliseconds.\n").
-spec send_after(subject(FNE), integer(), FNE) -> timer().
...
-file("src/gleam/erlang/process.gleam", 649).
...
-file("src/gleam/erlang/process.gleam", 673).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 689).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 701).
?DOC(
    " Sends an exit signal to a process, indicating that the process is to shut\n"
...
-file("src/gleam/erlang/process.gleam", 717).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 740).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 745).
...
-file("src/gleam/erlang/process.gleam", 760).
?DOC(
