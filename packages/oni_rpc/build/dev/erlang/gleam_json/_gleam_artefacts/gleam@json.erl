...
-spec decode_bits(
    bitstring(),
    fun((gleam@dynamic:dynamic_()) -> {ok, FTA} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, FTA} | {error, decode_error()}.
...
-spec do_decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, FSW} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, FSW} | {error, decode_error()}.
...
-spec decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, FSS} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, FSS} | {error, decode_error()}.
decode(Json, Decoder) ->
    do_decode(Json, Decoder).
...
-spec nullable(gleam@option:option(FTG), fun((FTG) -> json())) -> json().
...
-spec array(list(FTK), fun((FTK) -> json())) -> json().
