...
-spec map({ok, BIY} | {error, BIZ}, fun((BIY) -> BJC)) -> {ok, BJC} |
    {error, BIZ}.
map(Result, Fun) ->
    case Result of
        {ok, X} ->
...
-spec map_error({ok, BJF} | {error, BJG}, fun((BJG) -> BJJ)) -> {ok, BJF} |
    {error, BJJ}.
...
-spec flatten({ok, {ok, BJM} | {error, BJN}} | {error, BJN}) -> {ok, BJM} |
    {error, BJN}.
flatten(Result) ->
    case Result of
        {ok, X} ->
...
-spec 'try'({ok, BJU} | {error, BJV}, fun((BJU) -> {ok, BJY} | {error, BJV})) -> {ok,
        BJY} |
    {error, BJV}.
'try'(Result, Fun) ->
    case Result of
...
-spec then({ok, BKD} | {error, BKE}, fun((BKD) -> {ok, BKH} | {error, BKE})) -> {ok,
        BKH} |
    {error, BKE}.
then(Result, Fun) ->
    'try'(Result, Fun).
...
-spec unwrap({ok, BKM} | {error, any()}, BKM) -> BKM.
...
-spec lazy_unwrap({ok, BKQ} | {error, any()}, fun(() -> BKQ)) -> BKQ.
...
-spec unwrap_error({ok, any()} | {error, BKV}, BKV) -> BKV.
...
-spec unwrap_both({ok, BKY} | {error, BKY}) -> BKY.
unwrap_both(Result) ->
    case Result of
        {ok, A} ->
            A;
...
-file("src/gleam/result.gleam", 279).
?DOC(
...
-spec nil_error({ok, BLB} | {error, any()}) -> {ok, BLB} | {error, nil}.
nil_error(Result) ->
...
-spec 'or'({ok, BLH} | {error, BLI}, {ok, BLH} | {error, BLI}) -> {ok, BLH} |
    {error, BLI}.
'or'(First, Second) ->
    case First of
        {ok, _} ->
...
-file("src/gleam/result.gleam", 338).
?DOC(
...
-spec lazy_or({ok, BLP} | {error, BLQ}, fun(() -> {ok, BLP} | {error, BLQ})) -> {ok,
        BLP} |
    {error, BLQ}.
...
-spec all(list({ok, BLX} | {error, BLY})) -> {ok, list(BLX)} | {error, BLY}.
all(Results) ->
    gleam@list:try_map(Results, fun(X) -> X end).

-file("src/gleam/result.gleam", 384).
-spec do_partition(list({ok, BMM} | {error, BMN}), list(BMM), list(BMN)) -> {list(BMM),
    list(BMN)}.
...
-file("src/gleam/result.gleam", 380).
?DOC(
...
-spec partition(list({ok, BMF} | {error, BMG})) -> {list(BMF), list(BMG)}.
partition(Results) ->
    do_partition(Results, [], []).

-file("src/gleam/result.gleam", 406).
?DOC(
...
-spec replace({ok, any()} | {error, BMV}, BMY) -> {ok, BMY} | {error, BMV}.
...
-spec replace_error({ok, BNB} | {error, any()}, BNF) -> {ok, BNB} | {error, BNF}.
...
-file("src/gleam/result.gleam", 443).
?DOC(
...
-spec values(list({ok, BNI} | {error, any()})) -> list(BNI).
values(Results) ->
    gleam@list:filter_map(Results, fun(R) -> R end).

-file("src/gleam/result.gleam", 474).
?DOC(
    " Updates a value held within the `Error` of a result by calling a given function\n"
