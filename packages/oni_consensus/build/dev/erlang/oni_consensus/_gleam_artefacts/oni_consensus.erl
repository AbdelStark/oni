...
-spec list_nth(list(AJE), integer()) -> {ok, AJE} | {error, nil}.
list_nth(Lst, N) ->
    case {Lst, N} of
        {[], _} ->
            {error, nil};
...
-spec list_remove_nth_loop(list(AJN), integer(), list(AJN)) -> {ok,
        {AJN, list(AJN)}} |
    {error, nil}.
...
-spec list_remove_nth(list(AJI), integer()) -> {ok, {AJI, list(AJI)}} |
    {error, nil}.
