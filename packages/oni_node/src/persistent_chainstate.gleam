...
import gleam/result
import oni_bitcoin.{
  type Block, type BlockHash, type BlockHeader, type Network,
  type OutPoint, type Transaction, type TxOut,
}
import oni_storage.{
...
pub fn start(
  ...
  case persistent_storage.persistent_storage_open(storage_path) {
    Error(storage_err) -> {
      Error("Failed to open persistent storage: " <> storage_error_to_string(storage_err))
    }
    Ok(storage) -> {
      ...
      case load_or_init_chainstate(storage, config.network) {
        ...
        Ok(#(tip_hash, tip_height)) -> {
          let initial_state = PersistentChainstateState(
            config: config,
            storage: storage,
            tip_hash: tip_hash,
          ...
          case actor.start(initial_state, handle_message) {
            Ok(subject) -> {
              io.println("[Chainstate] Started at height " <> int.to_string(tip_height))
              Ok(subject)
            }
            Error(_) -> {
              let _ = persistent_storage.persistent_storage_close(storage)
...
fn load_or_init_chainstate(
  ...
  case persistent_storage.chainstate_get(storage) {
    Ok(chainstate) -> {
      ...
      io.println("[Chainstate] Loaded existing state at height " <>
    ...
    Error(oni_storage.NotFound) -> {
      ...
      let genesis_chainstate = oni_storage.Chainstate(
        best_block: params.genesis_hash,
        best_height: 0,
        total_tx: 1,
        total_coins: 1,
        total_amount: 5_000_000_000,  // 50 BTC initial subsidy
        pruned: False,
        pruned_height: None,
      )
      ...
      case persistent_storage.chainstate_put(storage, genesis_chainstate) {
        ...
        Error(e) -> Error("Failed to save genesis chainstate: " <> storage_error_to_string(e))
      }
    }
    Error(e) -> {
  ...
  case msg {
    ...
    GetInfo(reply) -> {
      ...
      let info = ChainstateInfo(
        ...
        total_tx: 0,  // Would need to track this
        total_coins: total_coins,
        is_synced: True,  // Placeholder
      )
    ...
    GetBlockIndex(hash, reply) -> {
      let result = case persistent_storage.block_index_get(state.storage, hash) {
        Ok(entry) -> Some(entry)
        Error(_) -> None
      }
      process.send(reply, result)
    ...
    Sync(reply) -> {
      case persistent_storage.persistent_storage_sync(state.storage) {
        Ok(_) -> {
          process.send(reply, Ok(Nil))
          actor.continue(PersistentChainstateState(..state, blocks_since_sync: 0))
        }
        Error(e) -> {
...
fn connect_block_persistent(
  ...
  case state.tip_hash {
    Some(tip_hash) -> {
      case oni_bitcoin.block_hash_to_hex(block.header.prev_block) ==
  ...
  case process_block_transactions(block, new_height, state.storage) {
    ...
    Ok(#(additions, removals, block_undo)) -> {
      ...
      case persistent_storage.utxo_batch(state.storage, additions, removals) {
        Error(e) -> Error("Failed to apply UTXO batch: " <> storage_error_to_string(e))
        Ok(_) -> {
          // Store undo data
          case persistent_storage.undo_put(state.storage, block_hash, block_undo) {
            Error(e) -> Error("Failed to store undo data: " <> storage_error_to_string(e))
            Ok(_) -> {
              // Update block index
              let entry = create_block_index_entry(block.header, block_hash, new_height, block)
              case persistent_storage.block_index_put(state.storage, entry) {
                Error(e) -> Error("Failed to update block index: " <> storage_error_to_string(e))
                Ok(_) -> {
                  // Update chainstate
                  case update_chainstate(state.storage, block_hash, new_height) {
                    Error(e) -> Error("Failed to update chainstate: " <> storage_error_to_string(e))
                    Ok(_) -> {
                      // Check if we need to sync
                      let new_blocks_since_sync = state.blocks_since_sync + 1
                      let should_sync = new_blocks_since_sync >= state.config.sync_interval

                      case should_sync {
                        True -> {
                          let _ = persistent_storage.persistent_storage_sync(state.storage)
                          Ok(PersistentChainstateState(
                            ..state,
  ...
  case state.tip_hash, state.tip_height > 0 {
    Some(tip_hash), True -> {
      ...
      case persistent_storage.undo_get(state.storage, tip_hash) {
        Error(e) -> Error("Failed to get undo data: " <> storage_error_to_string(e))
        Ok(block_undo) -> {
          // Get block index entry to find prev hash
          case persistent_storage.block_index_get(state.storage, tip_hash) {
            Error(e) -> Error("Failed to get block index: " <> storage_error_to_string(e))
            Ok(entry) -> {
              ...
              case apply_undo(state.storage, block_undo, entry) {
                ...
                Ok(_) -> {
                  // Delete undo data
                  let _ = persistent_storage.undo_delete(state.storage, tip_hash)

                  // Update chainstate
                  let new_height = state.tip_height - 1
                  case update_chainstate(state.storage, entry.prev_hash, new_height) {
                    Error(e) -> Error("Failed to update chainstate: " <> storage_error_to_string(e))
                    Ok(_) -> {
                      Ok(PersistentChainstateState(
                        ..state,
...
fn process_block_transactions(
  ...
  process_txs(block.transactions, height, storage, [], [], oni_storage.block_undo_new(), 0)
}
    ...
    [tx, ..rest] -> {
      ...
      case process_single_tx(tx, height, storage, is_coinbase) {
        ...
        Ok(#(tx_additions, tx_removals, tx_undo)) -> {
          ...
          process_txs(rest, height, storage, new_additions, new_removals, new_block_undo, tx_index + 1)
        }
      }
    }
  }
...
fn process_single_tx(
  ...
  let #(removals, tx_undo) = case is_coinbase {
    ...
    False -> collect_spent_coins(tx.inputs, storage, [], oni_storage.tx_undo_new())
  }
  ...
  case inputs {
    ...
    [input, ..rest] -> {
      ...
      let new_undo = case persistent_storage.utxo_get(storage, input.prevout) {
        ...
        Error(_) -> tx_undo  // Coin not found (shouldn't happen in valid chain)
      }
...
fn create_outputs(
  ...
  case outputs {
    ...
    [output, ..rest] -> {
      ...
      let coin = oni_storage.Coin(output: output, height: height, is_coinbase: is_coinbase)
      create_outputs(rest, txid, height, is_coinbase, vout + 1, [#(outpoint, coin), ..acc])
    }
  }
}
...
fn create_block_index_entry(
  ...
  oni_storage.BlockIndexEntry(
    ...
    total_work: 0,  // Would calculate cumulative work
    file_pos: None,
    undo_pos: None,
...
fn update_chainstate(
  ...
  let chainstate = oni_storage.Chainstate(
    ...
    total_tx: 0,  // Would track this
    total_coins: coin_count,
    total_amount: 0,  // Would track this
    pruned: False,
    pruned_height: None,
  )
