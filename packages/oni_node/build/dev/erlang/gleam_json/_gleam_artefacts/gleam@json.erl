...
-spec decode_bits(
    bitstring(),
    fun((gleam@dynamic:dynamic_()) -> {ok, FVI} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, FVI} | {error, decode_error()}.
...
-spec do_decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, FVE} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, FVE} | {error, decode_error()}.
...
-spec decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, FVA} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, FVA} | {error, decode_error()}.
decode(Json, Decoder) ->
    do_decode(Json, Decoder).
...
-spec nullable(gleam@option:option(FVO), fun((FVO) -> json())) -> json().
...
-spec array(list(FVS), fun((FVS) -> json())) -> json().
