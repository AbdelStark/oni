...
-type message(FXK) :: {message, FXK} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-type next(FXL, FXM) :: {continue,
        FXM,
        gleam@option:option(gleam@erlang@process:selector(FXL))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type init_result(FXN, FXO) :: {ready, FXN, gleam@erlang@process:selector(FXO)} |
    {failed, binary()}.

-type self(FXP, FXQ) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        FXP,
        gleam@erlang@process:subject(FXQ),
        gleam@erlang@process:selector(message(FXQ)),
        gleam@otp@system:debug_state(),
        fun((FXQ, FXP) -> next(FXQ, FXP))}.

-type spec(FXR, FXS) :: {spec,
        fun(() -> init_result(FXR, FXS)),
        integer(),
        fun((FXS, FXR) -> next(FXS, FXR))}.

-type start_error() :: init_timeout |
    {init_failed, gleam@erlang@process:exit_reason()} |
    {init_crashed, gleam@dynamic:dynamic_()}.

-type start_init_message(FXT) :: {ack,
        {ok, gleam@erlang@process:subject(FXT)} |
            {error, gleam@erlang@process:exit_reason()}} |
    {mon, gleam@erlang@process:process_down()}.

-file("src/gleam/otp/actor.gleam", 176).
-spec continue(FYA) -> next(any(), FYA).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(FYE, FYF), gleam@erlang@process:selector(FYE)) -> next(FYE, FYF).
...
-spec selecting_system_messages(gleam@erlang@process:selector(message(FYQ))) -> gleam@erlang@process:selector(message(FYQ)).
...
-spec receive_message(self(any(), FYM)) -> message(FYM).
...
-spec init_selector(
    gleam@erlang@process:subject(GCV),
    gleam@erlang@process:selector(GCV)
) -> gleam@erlang@process:selector(message(GCV)).
...
-spec initialise_actor(
    spec(any(), FZH),
    gleam@erlang@process:subject({ok, gleam@erlang@process:subject(FZH)} |
        {error, gleam@erlang@process:exit_reason()})
...
-spec start_spec(spec(any(), FZU)) -> {ok, gleam@erlang@process:subject(FZU)} |
    {error, start_error()}.
start_spec(Spec) ->
...
-spec start(GAA, fun((GAB, GAA) -> next(GAB, GAA))) -> {ok,
        gleam@erlang@process:subject(GAB)} |
    {error, start_error()}.
start(State, Loop) ->
    start_spec(
        {spec,
...
-spec send(gleam@erlang@process:subject(GAH), GAH) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(GAJ),
    fun((gleam@erlang@process:subject(GAL)) -> GAJ),
    integer()
) -> GAL.
