...
-opaque task(GWE) :: {task,
        gleam@erlang@process:pid_(),
        gleam@erlang@process:pid_(),
        gleam@erlang@process:process_monitor(),
        gleam@erlang@process:selector(message(GWE))}.

-type await_error() :: timeout | {exit, gleam@dynamic:dynamic_()}.

-type message(GWF) :: {from_monitor, gleam@erlang@process:process_down()} |
    {from_subject, GWF}.
...
-spec try_await(task(GWK), integer()) -> {ok, GWK} | {error, await_error()}.
...
-spec await(task(GWO), integer()) -> GWO.
await(Task, Timeout) ->
...
-spec try_await_forever(task(GWQ)) -> {ok, GWQ} | {error, await_error()}.
...
-spec await_forever(task(GWU)) -> GWU.
await_forever(Task) ->
