...
-type spec(GJV, GJW) :: {spec,
        GJV,
        integer(),
        integer(),
        fun((children(GJV)) -> children(GJW))}.

-opaque children(GJX) :: {ready, starter(GJX)} | {failed, child_start_error()}.

-opaque child_spec(GJY, GJZ, GKA) :: {child_spec,
        fun((GJZ) -> {ok, gleam@erlang@process:subject(GJY)} |
            {error, gleam@otp@actor:start_error()}),
        fun((GJZ, gleam@erlang@process:subject(GJY)) -> GKA)}.
...
-spec start_child(child_spec(any(), GKH, GKI), GKH) -> {ok, child(GKI)} |
    {error, child_start_error()}.
...
-spec perform_instruction_for_child(
    GKV,
    instruction(),
    child_spec(any(), GKV, GKX),
    child(GKX)
) -> {ok, {child(GKX), instruction()}} | {error, child_start_error()}.
...
-spec start_and_add_child(starter(GLO), child_spec(any(), GLO, GLR)) -> children(GLR).
...
-spec add(children(GLW), child_spec(any(), GLW, GLZ)) -> children(GLZ).
...
-spec supervisor(
    fun((GME) -> {ok, gleam@erlang@process:subject(GMF)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(GMF, GME, GME).
supervisor(Start) ->
...
-spec worker(
    fun((GMM) -> {ok, gleam@erlang@process:subject(GMN)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(GMN, GMM, GMM).
worker(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.
...
-spec returning(
    child_spec(GMU, GMV, any()),
    fun((GMV, gleam@erlang@process:subject(GMU)) -> GNB)
) -> child_spec(GMU, GMV, GNB).
returning(Child, Updater) ->
    {child_spec, erlang:element(2, Child), Updater}.

-file("src/gleam/otp/supervisor.gleam", 241).
-spec init(spec(any(), GNG)) -> gleam@otp@actor:init_result(state(GNG), message()).
init(Spec) ->
...
-spec handle_exit(gleam@erlang@process:pid_(), state(GNM)) -> gleam@otp@actor:next(message(), state(GNM)).
...
-spec loop(message(), state(GNR)) -> gleam@otp@actor:next(message(), state(GNR)).
loop(Message, State) ->
    case Message of
        {exit, Exit_message} ->
