...
-opaque queue(EQR) :: {queue, list(EQR), list(EQR)}.
...
-spec from_list(list(EQU)) -> queue(EQU).
from_list(List) ->
    {queue, [], List}.
...
-spec to_list(queue(EQX)) -> list(EQX).
to_list(Queue) ->
...
-spec push_back(queue(ERE), ERE) -> queue(ERE).
...
-spec push_front(queue(ERH), ERH) -> queue(ERH).
...
-spec pop_back(queue(ERK)) -> {ok, {ERK, queue(ERK)}} | {error, nil}.
pop_back(Queue) ->
    case Queue of
        {queue, [], []} ->
            {error, nil};
...
-spec pop_front(queue(ERP)) -> {ok, {ERP, queue(ERP)}} | {error, nil}.
pop_front(Queue) ->
    case Queue of
        {queue, [], []} ->
            {error, nil};
...
-spec reverse(queue(ERU)) -> queue(ERU).
reverse(Queue) ->
...
-spec is_logically_equal(queue(ESC), queue(ESC), fun((ESC, ESC) -> boolean())) -> boolean().
...
-spec is_equal(queue(ESF), queue(ESF)) -> boolean().
is_equal(A, B) ->
    check_equal(
        erlang:element(3, A),
        erlang:element(2, A),
