...
pub fn chainstate_start_test() {
  ...
  let result = supervisor.start_chainstate(oni_bitcoin.Mainnet)
  should.be_ok(result)
}

pub fn chainstate_initial_height_test() {
  // Test that chainstate starts at height 0
  let assert Ok(subject) = supervisor.start_chainstate(oni_bitcoin.Mainnet)
  let height = process.call(subject, supervisor.GetHeight, 1000)
...
pub fn chainstate_initial_tip_test() {
  ...
  let assert Ok(subject) = supervisor.start_chainstate(oni_bitcoin.Mainnet)
  let tip = process.call(subject, supervisor.GetTip, 1000)
...
pub fn chainstate_testnet_start_test() {
  ...
  let result = supervisor.start_chainstate(oni_bitcoin.Testnet)
  should.be_ok(result)
}

pub fn chainstate_regtest_start_test() {
  // Test that regtest chainstate starts successfully
  let result = supervisor.start_chainstate(oni_bitcoin.Regtest)
  should.be_ok(result)
}
...
pub fn mempool_start_test() {
  ...
  let result = supervisor.start_mempool(1000)
  should.be_ok(result)
}

pub fn mempool_initial_size_test() {
  // Test that mempool starts empty
  let assert Ok(subject) = supervisor.start_mempool(1000)
  let size = process.call(subject, supervisor.GetSize, 1000)
  should.equal(size, 0)
}
...
pub fn mempool_initial_txids_test() {
  ...
  let assert Ok(subject) = supervisor.start_mempool(1000)
  let txids = process.call(subject, supervisor.GetTxids, 1000)
  should.equal(txids, [])
}
...
pub fn mempool_add_invalid_tx_test() {
  ...
  let result = process.call(subject, supervisor.AddTx(invalid_tx, _), 1000)
...
pub fn mempool_size_limit_test() {
  ...
  let assert Ok(subject) = supervisor.start_mempool(0)  // Zero size limit
  ...
  let result = process.call(subject, supervisor.AddTx(tx, _), 1000)
...
pub fn sync_start_test() {
  ...
  let result = supervisor.start_sync()
  should.be_ok(result)
}

pub fn sync_initial_state_test() {
  // Test that sync coordinator starts in IBD state
  let assert Ok(subject) = supervisor.start_sync()
  let state = process.call(subject, supervisor.GetSyncState, 1000)
...
pub fn node_handles_creation_test() {
  ...
  let assert Ok(mempool) = supervisor.start_mempool(10_000)
  let assert Ok(sync) = supervisor.start_sync()

  let handles = supervisor.NodeHandles(
  ...
  let height = process.call(handles.chainstate, supervisor.GetHeight, 1000)
  let size = process.call(handles.mempool, supervisor.GetSize, 1000)
  let state = process.call(handles.sync, supervisor.GetSyncState, 1000)
...
pub fn utxo_not_found_test() {
  ...
  let assert Ok(subject) = supervisor.start_chainstate(oni_bitcoin.Regtest)
  ...
  let result = process.call(subject, supervisor.GetUtxo(outpoint, _), 1000)
