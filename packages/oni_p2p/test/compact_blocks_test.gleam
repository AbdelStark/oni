...
pub fn update_peer_state_test() {
  ...
  let msg = compact_blocks.SendCmpct(announce: True, version: 2)
...
pub fn decode_sendcmpct_no_announce_test() {
  let msg = compact_blocks.SendCmpct(announce: False, version: 1)
...
pub fn create_compact_block_test() {
  ...
  let block = oni_bitcoin.Block(header: header, transactions: [coinbase])

  let compact = compact_blocks.create_compact_block(block, 12_345, False)
...
pub fn create_compact_block_with_txs_test() {
  ...
  let tx2 =
    oni_bitcoin.Transaction(version: 3, inputs: [], outputs: [], lock_time: 0)

  let block =
    oni_bitcoin.Block(header: header, transactions: [coinbase, tx1, tx2])
...
pub fn getblocktxn_encode_decode_test() {
  let request =
    compact_blocks.BlockTxnRequest(
