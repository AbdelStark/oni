...
fn add_peer(network: SimNetwork, peer: SimPeer) -> SimNetwork {
  SimNetwork(..network, peers: dict.insert(network.peers, peer.id, peer))
}
...
pub fn handshake_initiation_test() {
  ...
  let msgs =
    oni_p2p.handshake_initiate(services, 1_700_000_000, addr, 12_345, 800_000)
  ...
  case msgs {
    [oni_p2p.MsgVersion(payload)] -> {
      ...
      payload.start_height |> should.equal(800_000)
    }
    _ -> should.fail()
  }
}
...
pub fn multiple_peer_connection_test() {
  ...
  let network =
    sim_network_new()
    |> add_peer(sim_peer("1", 800_000))
    |> add_peer(sim_peer("2", 800_100))
    |> add_peer(sim_peer("3", 799_900))

  // Connect all peers
  let connected_network =
    SimNetwork(
      ..network,
      peers: dict.map_values(network.peers, fn(_id, peer) { connect_peer(peer) }),
    )
...
pub fn peer_disconnection_handling_test() {
  ...
  let network =
    sim_network_new()
    |> add_peer(connect_peer(sim_peer("1", 800_000)))
    |> add_peer(connect_peer(sim_peer("2", 800_100)))
...
pub fn tx_inv_relay_test() {
  ...
  let #(_tx_relay3, msg_opt) =
    relay.tx_relay_get_inv(tx_relay2, peer, 1_700_000_000)
...
pub fn inv_deduplication_test() {
  ...
  let #(_tx_relay4, msg2) =
    relay.tx_relay_get_inv(tx_relay3, peer, 1_700_001_000)
  ...
  case msg2 {
    ...
    // Should be None - already announced
    Some(_) -> should.be_ok(Ok(Nil))
    // Or new txs queued
  }
}
...
pub fn immediate_ban_test() {
  ...
  case action {
    ...
    _ -> should.fail()
    // Should ban or disconnect for invalid PoW
  }
...
pub fn eclipse_attack_defense_test() {
  ...
  let honest_peers =
    list.range(1, 8)
    |> list.map(fn(i) { sim_peer(int.to_string(i), 800_000) })
...
pub fn sybil_attack_defense_test() {
  ...
  let peers_with_incidents =
    list.range(1, 100)
    |> list.map(fn(i) { "peer" <> int.to_string(i) })

  // Each peer sends a duplicate message
  let final_manager =
    list.fold(peers_with_incidents, manager, fn(mgr, peer_id) {
      let #(new_mgr, _action) =
        ban_manager.record_misbehavior(
          mgr,
          peer_id,
...
pub fn many_peers_test() {
  ...
  let peers =
    list.range(1, 100)
    |> list.map(fn(i) { sim_peer(int.to_string(i), 800_000 + i) })
