...
-type message(FUZ) :: {message, FUZ} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-type next(FVA, FVB) :: {continue,
        FVB,
        gleam@option:option(gleam@erlang@process:selector(FVA))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type init_result(FVC, FVD) :: {ready, FVC, gleam@erlang@process:selector(FVD)} |
    {failed, binary()}.

-type self(FVE, FVF) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        FVE,
        gleam@erlang@process:subject(FVF),
        gleam@erlang@process:selector(message(FVF)),
        gleam@otp@system:debug_state(),
        fun((FVF, FVE) -> next(FVF, FVE))}.

-type spec(FVG, FVH) :: {spec,
        fun(() -> init_result(FVG, FVH)),
        integer(),
        fun((FVH, FVG) -> next(FVH, FVG))}.

-type start_error() :: init_timeout |
    {init_failed, gleam@erlang@process:exit_reason()} |
    {init_crashed, gleam@dynamic:dynamic_()}.

-type start_init_message(FVI) :: {ack,
        {ok, gleam@erlang@process:subject(FVI)} |
            {error, gleam@erlang@process:exit_reason()}} |
    {mon, gleam@erlang@process:process_down()}.

-file("src/gleam/otp/actor.gleam", 176).
-spec continue(FVP) -> next(any(), FVP).
continue(State) ->
    {continue, State, none}.

-file("src/gleam/otp/actor.gleam", 184).
?DOC(
...
-spec with_selector(next(FVT, FVU), gleam@erlang@process:selector(FVT)) -> next(FVT, FVU).
...
-file("src/gleam/otp/actor.gleam", 257).
-spec exit_process(gleam@erlang@process:exit_reason()) -> gleam@erlang@process:exit_reason().
exit_process(Reason) ->
    Reason.

-file("src/gleam/otp/actor.gleam", 293).
-spec selecting_system_messages(gleam@erlang@process:selector(message(FWF))) -> gleam@erlang@process:selector(message(FWF)).
...
-file("src/gleam/otp/actor.gleam", 262).
-spec receive_message(self(any(), FWB)) -> message(FWB).
...
-file("src/gleam/otp/actor.gleam", 306).
...
-file("src/gleam/otp/actor.gleam", 413).
-spec init_selector(
    gleam@erlang@process:subject(GAK),
    gleam@erlang@process:selector(GAK)
) -> gleam@erlang@process:selector(message(GAK)).
...
-file("src/gleam/otp/actor.gleam", 316).
...
-file("src/gleam/otp/actor.gleam", 372).
-spec initialise_actor(
    spec(any(), FWW),
    gleam@erlang@process:subject({ok, gleam@erlang@process:subject(FWW)} |
        {error, gleam@erlang@process:exit_reason()})
...
-file("src/gleam/otp/actor.gleam", 442).
?DOC(
...
-file("src/gleam/otp/actor.gleam", 464).
?DOC(
...
-spec start_spec(spec(any(), FXJ)) -> {ok, gleam@erlang@process:subject(FXJ)} |
    {error, start_error()}.
start_spec(Spec) ->
...
-file("src/gleam/otp/actor.gleam", 512).
?DOC(
...
-spec start(FXP, fun((FXQ, FXP) -> next(FXQ, FXP))) -> {ok,
        gleam@erlang@process:subject(FXQ)} |
    {error, start_error()}.
start(State, Loop) ->
    start_spec(
        {spec,
...
-spec call(
    gleam@erlang@process:subject(FXY),
    fun((gleam@erlang@process:subject(FYA)) -> FXY),
    integer()
) -> FYA.
