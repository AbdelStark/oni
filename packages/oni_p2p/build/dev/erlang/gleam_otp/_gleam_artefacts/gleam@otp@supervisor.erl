...
-define(FILEPATH, "src/gleam/otp/supervisor.gleam").
...
-type spec(GHK, GHL) :: {spec,
        GHK,
        integer(),
        integer(),
        fun((children(GHK)) -> children(GHL))}.

-opaque children(GHM) :: {ready, starter(GHM)} | {failed, child_start_error()}.

-opaque child_spec(GHN, GHO, GHP) :: {child_spec,
        fun((GHO) -> {ok, gleam@erlang@process:subject(GHN)} |
            {error, gleam@otp@actor:start_error()}),
        fun((GHO, gleam@erlang@process:subject(GHN)) -> GHP)}.
...
-file("src/gleam/otp/supervisor.gleam", 82).
-spec start_child(child_spec(any(), GHW, GHX), GHW) -> {ok, child(GHX)} |
    {error, child_start_error()}.
...
-file("src/gleam/otp/supervisor.gleam", 101).
-spec shutdown_child(
    gleam@erlang@process:pid_(),
...
-file("src/gleam/otp/supervisor.gleam", 105).
-spec perform_instruction_for_child(
    GIK,
    instruction(),
    child_spec(any(), GIK, GIM),
    child(GIM)
) -> {ok, {child(GIM), instruction()}} | {error, child_start_error()}.
...
-file("src/gleam/otp/supervisor.gleam", 127).
-spec add_child_to_starter(
    starter(GIU),
    child_spec(any(), GIU, GIX),
    child(GIX)
) -> starter(GIX).
...
-file("src/gleam/otp/supervisor.gleam", 157).
-spec start_and_add_child(starter(GJD), child_spec(any(), GJD, GJG)) -> children(GJG).
...
-file("src/gleam/otp/supervisor.gleam", 199).
?DOC(
...
-spec supervisor(
    fun((GJT) -> {ok, gleam@erlang@process:subject(GJU)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(GJU, GJT, GJT).
supervisor(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.

-file("src/gleam/otp/supervisor.gleam", 221).
?DOC(
...
-spec worker(
    fun((GKB) -> {ok, gleam@erlang@process:subject(GKC)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(GKC, GKB, GKB).
worker(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.

-file("src/gleam/otp/supervisor.gleam", 234).
?DOC(
...
-spec returning(
    child_spec(GKJ, GKK, any()),
    fun((GKK, gleam@erlang@process:subject(GKJ)) -> GKQ)
) -> child_spec(GKJ, GKK, GKQ).
returning(Child, Updater) ->
    {child_spec, erlang:element(2, Child), Updater}.

-file("src/gleam/otp/supervisor.gleam", 241).
-spec init(spec(any(), GKV)) -> gleam@otp@actor:init_result(state(GKV), message()).
init(Spec) ->
...
-file("src/gleam/otp/supervisor.gleam", 298).
-spec handle_exit(gleam@erlang@process:pid_(), state(GLB)) -> gleam@otp@actor:next(message(), state(GLB)).
handle_exit(Pid, State) ->
    Outcome = begin
        Start@1 = case erlang:element(3, erlang:element(3, State)) of
            {some, Start} -> Start;
            _assert_fail ->
        ...
        gleam@result:then(
            ...
            fun(Restarts) ->
                gleam@result:then(
                    begin
                        _pipe@2 = Start@1({start_from, Pid}),
                        gleam@result:map_error(
                            _pipe@2,
                            fun(E) ->
                                {restart_failed,
...
-file("src/gleam/otp/supervisor.gleam", 338).
-spec loop(message(), state(GLG)) -> gleam@otp@actor:next(message(), state(GLG)).
loop(Message, State) ->
    case Message of
        {exit, Exit_message} ->
...
-file("src/gleam/otp/supervisor.gleam", 370).
?DOC(
...
-file("src/gleam/otp/supervisor.gleam", 401).
?DOC(
