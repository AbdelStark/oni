...
pub fn pubkey_compressed_odd_test() {
  ...
  let bytes = <<
    0x03, 0x79, 0xBE, 0x66, 0x7E, 0xF9, 0xDC, 0xBB, 0xAC, 0x55, 0xA0, 0x62, 0x95,
    0xCE, 0x87, 0x0B, 0x07, 0x02, 0x9B, 0xFC, 0xDB, 0x2D, 0xCE, 0x28, 0xD9, 0x59,
    0xF2, 0x81, 0x5B, 0x16, 0xF8, 0x17, 0x98,
  >>
  let result = oni_bitcoin.pubkey_from_bytes(bytes)
...
pub fn pubkey_invalid_prefix_test() {
  // Invalid prefix 0x05
  let bytes = <<
    0x05, 0x79, 0xBE, 0x66, 0x7E, 0xF9, 0xDC, 0xBB, 0xAC, 0x55, 0xA0, 0x62, 0x95,
    0xCE, 0x87, 0x0B, 0x07, 0x02, 0x9B, 0xFC, 0xDB, 0x2D, 0xCE, 0x28, 0xD9, 0x59,
    0xF2, 0x81, 0x5B, 0x16, 0xF8, 0x17, 0x98,
  >>
  let result = oni_bitcoin.pubkey_from_bytes(bytes)
...
pub fn bech32_encode_decode_test() {
  ...
  let data = [
    0, 14, 20, 15, 7, 13, 26, 0, 25, 18, 6, 11, 13, 8, 21, 4, 20, 3, 17, 2, 29,
    3, 12, 29, 3, 4, 15, 24, 20, 6, 14, 30, 22,
  ]
...
pub fn address_p2pkh_roundtrip_test() {
  let hash = <<
    0x14, 0x62, 0xe9, 0x07, 0xb1, 0x5c, 0xbf, 0x27, 0xd5, 0x42, 0x5f, 0x99, 0x7d,
    0xab, 0x5f, 0xec, 0x16, 0x24, 0xa3, 0x2e,
  >>
